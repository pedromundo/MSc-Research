...
void show_depth(uint16_t *depth) {
  ...
  for (i = 0; i < kinect.height; i++)
    for (j = 0; j < kinect.width; j++) {
      ...
      if (depth_in_mm != 0 && depth_in_mm >= depth_min &&
          depth_in_mm <= depth_max && i >= top_plane && i <= bottom_plane &&
          j >= left_plane && j <= right_plane) {
        uint16_t color = (uint16_t)lerp(
            255.0, 32.0, (double)depth_in_mm / FREENECT_DEPTH_MM_MAX_VALUE);
...
cv::Mat clean_image(cv::Mat input_image, int near, int far, int left, int right,
                    int top, int bottom) {
  cv::Mat output_image;
  ...
  for (int i = 0; i < output_image.size().height; ++i) {
    for (int j = 0; j < output_image.size().width; ++j) {
      ...
      if (pixel > far || pixel < near || i < top || i > bottom || j < left ||
          j > right) {
        pixel = 0;
      }
    }
...
int save_depth(uint16_t *depth) {
  ...
  depth_mat = clean_image(depth_mat, depth_min, depth_max, left_plane,
...
int save_depth_burst(uint16_t *depth, unsigned int frame_count) {
  cv::Mat depth_mat = cv::Mat(kinect, CV_16UC1, depth);
  std::ostringstream oss;
  oss << capture_name << "_burst_" << count_burst * capture_step << "_"
      << frame_count << ".png";
  depth_mat = clean_image(depth_mat, depth_min, depth_max, left_plane,
                          right_plane, top_plane, bottom_plane);
  imwrite(oss.str(), depth_mat);
...
int save_ply(cv::Mat depth_mat, cv::Mat color_mat, int count) {
  ...
  depth_mat = cv::Mat(depth_mat_filtered);*/
  ...
  for (int i = 0; i < depth_mat.size().height; ++i) {
    for (int j = 0; j < depth_mat.size().width; ++j) {
      ...
      if (i > 0 && j > 0 && i < depth_mat.size().height - 1 &&
          j < depth_mat.size().width - 1) {
  ...
  for (int i = 0; i < depth_mat.size().height; i++) {
    for (int j = 0; j < depth_mat.size().width; j++) {
      ...
      if (z_in_mm != 0 && z_in_mm >= depth_min && z_in_mm <= depth_max &&
          i >= top_plane && i <= bottom_plane && j >= left_plane &&
          j <= right_plane) {
        ++num_vertices;
      }
    }
    ...
    for (int j = 0; j < depth_mat.size().width; j++) {
      ...
      if (z_in_mm != 0 && z_in_mm >= depth_min && z_in_mm <= depth_max &&
        ...
        fprintf(fp, "%.6lf %.6lf %.6lf %.6lf %.6lf %.6lf %d %d %d\n",
                ...
                (double)normal[1], (double)normal[2], color[0], color[1],
                color[2]);
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  cv::createTrackbar("Left Plane (px)", "DEPTH", &left_plane, kinect.width - 1,
                     NULL);
  cv::createTrackbar("Right Plane (px)", "DEPTH", &right_plane,
                     kinect.width - 1, NULL);
  cv::createTrackbar("Top Plane (px)", "DEPTH", &top_plane, kinect.height - 1,
                     NULL);
  cv::createTrackbar("Bottom Plane (px)", "DEPTH", &bottom_plane,
                     kinect.height - 1, NULL);
  ...
  while (key = cv::waitKey(1)) {
    ...
    // opencv apparently cant decide whether 'no key' is -1 or 255. (Yes, I
    // tried masking and casting)
    if (key == 255 || key == -1) {
      uint32_t timestamp;
...
#pragma omp parallel num_threads(2)
      {
        ...
        if (thread_num == 0) {
          ...
          freenect_sync_get_depth((void **)(&depth_data), &timestamp_depth, 0,
      ...
      if (key == DEPTH_MESH) {
        ...
        freenect_sync_get_video((void **)(&color_data), &timestamp_color, 0,
                                FREENECT_VIDEO_RGB);
        save_ply(cv::Mat(kinect, CV_16UC1, depth_data),
                 cv::Mat(kinect, CV_8UC3, color_data), count_depth_mesh);
        ++count_depth_mesh;
      }
      if (key == ALL) {
        // depth
        ...
        uint16_t *burst_data;
        for (unsigned int i = 0; i < SR_SIZE; ++i) {
          freenect_sync_get_depth((void **)(&burst_data), &timestamp_burst, 0,
                                  FREENECT_DEPTH_REGISTERED);
