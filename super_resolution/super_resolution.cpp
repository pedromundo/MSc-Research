...
constexpr int SR_SIZE = 16;
constexpr int RESAMPLE_FACTOR = 4;
...
int save_ply(cv::Mat depth_mat, std::string filename,
...
int main(int argc, char **argv) {
  ...
  cv::Mat lr_images[SR_SIZE];            // stored as float
  cv::Mat lr_images_upsampled[SR_SIZE];  // stored as float
  cv::Mat alignment_matrices[SR_SIZE];   // affine transform matrices
  cv::Mat hr_image;
...
#pragma omp parallel for
  for (size_t i = 0; i < SR_SIZE; ++i) {
    ...
    findTransformECC(
        template_image, lr_images[i], alignment_matrices[i], cv::MOTION_AFFINE,
        cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 200,
                         1E-12));
    std::cout << "Alignment " << i << "-> 0 = " << alignment_matrices[i]
              << std::endl;
  }
  ...
  for (size_t i = 0; i < SR_SIZE; ++i) {
    ...
    if (i > 0) {
      warpAffine(lr_images[i], lr_images[i], alignment_matrices[i],
                 lr_images[i].size(), cv::WARP_INVERSE_MAP);
    }

    // Upsample - can use pyramids or perform a simple scale operation
    // results were exactly the same, still gotta find out why
    cv::resize(lr_images[i], lr_images_upsampled[i],
               lr_images[i].size() * RESAMPLE_FACTOR, 0, 0, cv::INTER_NEAREST);
  }
  ...
  // Downsample the HR image back to 640x480 to keep it valid within the
  // coordinate system of the Microsoft Kinect sensor - lanczos removes some
  // artifacts with no apparent downsides
  cv::resize(hr_image, hr_image, hr_image.size() / RESAMPLE_FACTOR, 0, 0,
             cv::INTER_LANCZOS4);
