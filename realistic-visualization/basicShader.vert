...
out vec3 vertNormal_tangentspace;
...
void main() {
	...
	vec3 vertEyeDirection_cameraspace = normalize(eyePos - Position_cameraspace);

	// Vector that goes from the vertex to the light, in camera space. M is ommited because it's identity.
	vec3 LightPosition_cameraspace = ( MV * lightPos).xyz;
	vec3 vertLightDirection_cameraspace = LightPosition_cameraspace + vertEyeDirection_cameraspace;

	// Normal of the the vertex, in camera space
	vec3 vertNormal_cameraspace = ( MV * appNormal_modelspace).xyz;
	...
	vertNormal_tangentspace = TBN * vertNormal_cameraspace;
    ...
    vertTexCoord = vec2(appTexCoord.x,appTexCoord.y);
